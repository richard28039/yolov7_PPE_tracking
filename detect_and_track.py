...
def draw_boxes(img, bbox, identities=None, color=None , categories=None, names=None, save_with_object_id=False, path=None,offset=(0, 0)):
    for i, box in enumerate(bbox):
        ...
        cv2.rectangle(img, (x1, y1), (x2, y2), color[cat], 3, cv2.LINE_AA)
        cv2.circle(img,(center_x, center_y), 7, color[cat], -1)
        cv2.rectangle(img, (x1, y1 - 20), (x1 + w, y1), color[cat], -1)
...
def detect(save_img=False):
    ...
    colors = [class_colors[name] for name in names]
